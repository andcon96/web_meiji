...
public function saveOtherShipmentPreparation($approver, $ossmId, $otherShipmentPreparation, $activeConnection)
{
    ...
    try {
        ...
        foreach ($otherShipmentPreparation as $key => $shipmentPreparation) {
            ...
            if ($otherShipmentScheduleDet) {
                ...
                foreach ($shipmentPreparation['locations'] as $locationDetail) {
                    ...
                    // Insert ke history shipment schedule
                    ...
                    if ($qtyPick > 0) {
                        ...
                        // -------------------------------------------------------------
                        //  INSERT GENERAL TRANSACTION HISTORY
                        // -------------------------------------------------------------
                        $newTransactionHistory = new TransactionHistory();
                        $newTransactionHistory->tr_nbr       = $otherShipmentScheduleDet->getOtherShipmentScheduleMaster->ossm_number ?? '';
                        $newTransactionHistory->tr_order     = '';
                        $newTransactionHistory->tr_program   = 'Other Transaction Module';
                        $newTransactionHistory->tr_activity  = 'Preparation Other Transaction';
                        $newTransactionHistory->tr_user      = Auth::user()->id ?? '';
                        $newTransactionHistory->tr_part      = $shipmentPreparation['ossdPart'] ?? $otherShipmentScheduleDet->ossd_part;
                        $newTransactionHistory->tr_uom       = $otherShipmentScheduleDet->ossd_uom ?? '';
                        $newTransactionHistory->tr_line      = '';
                        $newTransactionHistory->tr_lot       = $locationDetail['lot'] ?? '';
                        $newTransactionHistory->tr_qty       = $qtyPick;
                        $newTransactionHistory->tr_date      = now();
                        $newTransactionHistory->tr_reference = $ossmId ?? '';
                        $newTransactionHistory->tr_site      = $locationDetail['site'] ?? '2100';
                        $newTransactionHistory->tr_location  = $locationDetail['loc'] ?? '';
                        $newTransactionHistory->tr_warehouse = $locationDetail['wh'] ?? '';
                        $newTransactionHistory->tr_level     = $locationDetail['level'] ?? '0';
                        $newTransactionHistory->tr_bin       = $locationDetail['bin'] ?? '0';
                        $newTransactionHistory->tr_remark    = 'Preparation Other Transaction';
        ...
        $otherShipmentScheduleMstr->ossm_status = 'Preparation';
    ...
    try {
        ...
        foreach ($otherShipmentPreparationMaster->getOtherShipmentPreparationDet as $prepDet) {
            ...
            if ($qtyPick > 0) {
                ...
                // -------------------------------------------------------------
                //  INSERT GENERAL TRANSACTION HISTORY (REJECT / ROLLBACK)
                // -------------------------------------------------------------
                $newTransactionHistory = new TransactionHistory();
                $newTransactionHistory->tr_nbr       = $otherShipmentScheduleNumber ?? '';
                $newTransactionHistory->tr_order     = '';
                $newTransactionHistory->tr_program   = 'Other Transaction Module';
                ...
                $newTransactionHistory->tr_line      = '';
                $newTransactionHistory->tr_lot       = $location->ossl_lotserial ?? '';
                $newTransactionHistory->tr_qty       = $qtyPick; // Kuantitas stok yang dikembalikan
                $newTransactionHistory->tr_date      = now();
                $newTransactionHistory->tr_reference = $otherShipmentPreparation['get_other_shipment_preparation_mstr']['ospm_number'] ?? '';
                $newTransactionHistory->tr_site      = $location->ossl_site ?? '2100';
                $newTransactionHistory->tr_location  = $location->ossl_location ?? '';
                $newTransactionHistory->tr_warehouse = $location->ossl_warehouse ?? '';
                $newTransactionHistory->tr_level     = $location->ossl_level ?? '0';
                $newTransactionHistory->tr_bin       = $location->ossl_bin ?? '0';
                $newTransactionHistory->tr_remark    = $reason ?? 'Rejected Other Shipment Preparation';
                $newTransactionHistory->save();
...
public function approveOtherShipmentPreparation($shipmentPreparation, $reason, $otherShipmentScheduleNumber, $activeConnection)
{
    ...
    try {
        ...
        $otherShipmentScheduleMaster->ossm_status = 'Approve';
        ...
        // -------------------------------------------------------------
        //  INSERT GENERAL TRANSACTION HISTORY (APPROVE)
        // -------------------------------------------------------------
        if ($otherShipmentScheduleMaster) {
            foreach ($otherShipmentScheduleMaster->getOtherShipmentScheduleDetail as $scheduleDet) {
                foreach ($scheduleDet->getOtherShipmentScheduleLocation as $location) {
                    if ((float)$location->ossl_qty_pick > 0) {
                        $newTransactionHistory = new TransactionHistory();
                        $newTransactionHistory->tr_nbr       = $otherShipmentScheduleNumber ?? '';
                        $newTransactionHistory->tr_order     = '';
                        $newTransactionHistory->tr_program   = 'Other Transaction Module';
                        $newTransactionHistory->tr_activity  = 'Approve Other Transaction  ';
                        $newTransactionHistory->tr_user      = Auth::user()->id ?? '';
                        $newTransactionHistory->tr_part      = $scheduleDet->ossd_part ?? '';
                        $newTransactionHistory->tr_uom       = $scheduleDet->ossd_uom ?? '';
                        $newTransactionHistory->tr_line      = '';
                        $newTransactionHistory->tr_lot       = $location->ossl_lotserial ?? '';
                        $newTransactionHistory->tr_qty       = $location->ossl_qty_pick;
                        $newTransactionHistory->tr_date      = now();
                        $newTransactionHistory->tr_reference = $shipmentPreparation['get_other_shipment_preparation_mstr']['ospm_number'] ?? '';
                        $newTransactionHistory->tr_site      = $location->ossl_site ?? '2100';
                        $newTransactionHistory->tr_location  = $location->ossl_location ?? '';
                        $newTransactionHistory->tr_warehouse = $location->ossl_warehouse ?? '';
                        $newTransactionHistory->tr_level     = $location->ossl_level ?? '0';
                        $newTransactionHistory->tr_bin       = $location->ossl_bin ?? '0';
                        $newTransactionHistory->tr_remark    = $reason ?? 'Approved Other Shipment Preparation';
                        $newTransactionHistory->save();
                    }
                }
            }
