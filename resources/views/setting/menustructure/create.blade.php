...
@section('content')
    <div class="container">
        <form action="{{ route('menuStructure.store') }}" method="POST" enctype="multipart/form-data">
            @method('POST')
            @csrf
            <div class="card mb-4">
                <h5 class="card-header">Create Menu Structure</h5>
                <div class="card-body">
                    <div class="mb-3 row">
                            ...
                            <select id="select2Icons" name="icon_id" class="select2-icons form-select" data-allow-clear="true">
                                ...
                                @foreach ($icons as $icon)
                                    <option value="{{ $icon->id }}" data-icon="{{$icon->icon_value}}"
                                        {{old('icon_id') == $icon->id ? 'selected' : ''}}>{{$icon->icon_name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-md-10">
                            <select id="formtabs-menuParent" name="menu_parent_id" class="select2 form-select" data-allow-clear="true">
                                ...
                                @foreach ($menuParents as $menuParent)
                                    <option value="{{ $menuParent->id }}" {{old('menu_parent_id') == $menuParent->id ? 'selected' : ''}}>{{ $menuParent->menu_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
